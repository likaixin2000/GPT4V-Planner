...
## 0.36330658197402954, [0, 0.1928356905883789]
    ...
    def pick(self, mask):
        ...
        mask = self.numpy_to_ros_image(mask, encoding="mono8")
        ...
        poses = grasping_service(self.rgb_image, self.depth_image, mask, K, 1)
    ...
    def ros_image_to_numpy(self, rgb_image):
        image_np = np.frombuffer(rgb_image.data, dtype=np.uint8).reshape(rgb_image.height, rgb_image.width, -1)
        return image_np
    
    def numpy_to_ros_image(self, np_img, encoding='rgb8'):
        ros_image = Image()
        ros_image.height = np_img.shape[0]
        ros_image.width = np_img.shape[1]
        ros_image.encoding = encoding  # Assuming the image is in RGB format
        ros_image.is_bigendian = 0   # Assuming the system is little endian
        ros_image.step = np_img.shape[1] * 3  # Step size is width times the number of channels
        ros_image.data = np_img.tostring()

        return ros_image

    def get_img(self):
        image_np = self.ros_image_to_numpy(self.rgb_image)

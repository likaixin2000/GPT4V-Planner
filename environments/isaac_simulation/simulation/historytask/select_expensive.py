...
class SelectExpensive(Task):
    ...
    def reset(self):
        ...
        self._env.add_object_relative_to_table("iphone","iphone/iphone.urdf",[0.1, -0.3, 0.2],[0., 0, 1, -0.5 * np.pi])

        # add distractor
        self._env.add_object_relative_to_table("cup","yellow_cup/model.urdf",[-0.1, -0.2, 0.05],[0., 0, 1, 0.5 * np.pi])

        self._env.add_object_relative_to_table("book","book_1/model.urdf",[0.1, 0.3, 0.1],[0.09, 0.09, 2, 0.5 * np.pi])

...
class SelectInLibrary(Task):
    ...
    def reset(self):
        ...
        self._env.add_object_relative_to_table("iphone","iphone/iphone.urdf",[0.1, -0.3, 0.05],[0., 0, 1, -0.5 * np.pi])

        # add distractor
        self._env.add_object_relative_to_table("cup","yellow_cup/model.urdf",[-0.1, -0.2, 0.05],[0., 0, 1, 0.5 * np.pi])

        self._env.add_object_relative_to_table("book","book_3/model.urdf",[0.1, 0.2, 0.15],[1,0,0, 0.5 * np.pi])

        self._env.set_look_down_degree_camera(45)
        self._env.empty_step(120)

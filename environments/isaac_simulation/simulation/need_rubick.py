...
class NeedRubick(Task):
    ...
    def reset(self):
        ...
        # set remote control
        self._env.add_object_relative_to_table("rubick","rubick/cube.urdf",[-0.1, -0.2, 0.2],[0.09, 0.09, 2, 0.5 * np.pi])

        # todo
        # self._env.add_object('lcdtv','lcdtv/tv.urdf',[0.5, 0.8, 0.8],[0, 0, 0,0],fix_base_link=True)

        # add distractor
        self._env.add_object_relative_to_table("bowl","bowl/model.urdf",[0.1, -0.2, 0.05],[0., 0, 1, 0.5 * np.pi])
        #绕x轴旋转90度，再绕z轴旋转90度
        ...
        self._env.add_object_relative_to_table_eular("marker","black_marker/model.urdf",[0, 0, 0.5],[0.5 * np.pi,0,0.5 * np.pi])

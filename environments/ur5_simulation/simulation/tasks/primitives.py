...
class PickPlace():
    ...
    def __call__(self, movej, movep, ee, pose0, pose1):
        ...
        prepick_pose = simulation_utils.multiply(pick_pose, prepick_to_pick)
        postpick_pose = simulation_utils.multiply(pick_pose, postpick_to_pick)
        timeout = movep(prepick_pose)

        # Move towards pick pose until contact is detected.
        delta = (np.float32([0, 0, -0.001]),
                 simulation_utils.eulerXYZ_to_quatXYZW((0, 0, 0)))
        targ_pose = prepick_pose
        while not ee.detect_contact():  # and target_pose[2] > 0:
            targ_pose = simulation_utils.multiply(targ_pose, delta)
        ...
        if pick_success:
            ...
            preplace_pose = simulation_utils.multiply(place_pose, preplace_to_place)
            postplace_pose = simulation_utils.multiply(place_pose, postplace_to_place)

...
class Task():
    ...
    def oracle(self, env):
        ...
        def act(obs, info):  # pylint: disable=unused-argument
            ...
            pick_pix = simulation_utils.sample_distribution(pick_prob)
            # For "deterministic" demonstrations on insertion-easy, use this:
            # pick_pix = (160,80)
            pick_pos = simulation_utils.pix_to_xyz(pick_pix, hmap,
            ...
            if not self.sixdof:
                obj_euler = simulation_utils.quatXYZW_to_eulerXYZ(obj_pose[1])
                obj_quat = simulation_utils.eulerXYZ_to_quatXYZW((0, 0, obj_euler[2]))
                obj_pose = (obj_pose[0], obj_quat)
            world_to_pick = simulation_utils.invert(pick_pose)
            obj_to_pick = simulation_utils.multiply(world_to_pick, obj_pose)
            pick_to_obj = simulation_utils.invert(obj_to_pick)
            place_pose = simulation_utils.multiply(targ_pose, pick_to_obj)
        ...
        # Evaluate by measuring object intersection with zone.
            ...
            for zone_idx, (zone_pose, zone_size) in enumerate(zones):
                ...
                for obj_idx, obj_id in enumerate(obj_pts):
                    ...
                    world_to_zone = simulation_utils.invert(zone_pose)
                    obj_to_zone = simulation_utils.multiply(world_to_zone, obj_pose)
                    pts = np.float32(simulation_utils.apply(obj_to_zone, pts))
                    if len(zone_size) > 1:
    ...
    def is_match(self, pose0, pose1, symmetry):
        ...
        diff_rot = 0
        if symmetry > 0:
            rot0 = np.array(simulation_utils.quatXYZW_to_eulerXYZ(pose0[1]))[2]
            rot1 = np.array(simulation_utils.quatXYZW_to_eulerXYZ(pose1[1]))[2]
    ...
    def get_true_image(self, env):
        ...
        hmaps, cmaps = simulation_utils.reconstruct_heightmaps(
    ...
    def get_random_pose(self, env, obj_size):
        ...
        if np.sum(free) == 0:
            return None, None
        pix = simulation_utils.sample_distribution(np.float32(free))
        pos = simulation_utils.pix_to_xyz(pix, hmap, self.bounds, self.pix_size)
        pos = (pos[0], pos[1], obj_size[2] / 2)
        theta = np.random.rand() * 2 * np.pi
        rot = simulation_utils.eulerXYZ_to_quatXYZW((0, 0, theta))
        return pos, rot

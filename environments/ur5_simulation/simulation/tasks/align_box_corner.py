...
class AlignBoxCorner(Task):
    ...
    def reset(self, env):
        ...
        theta = simulation_utils.quatXYZW_to_eulerXYZ(corner_pose[1])[2]
        fip_rot = simulation_utils.eulerXYZ_to_quatXYZW((0, 0, theta + np.pi))
        pose1 = (corner_pose[0], fip_rot)
        alt_x = (box_size[0] / 2) - (box_size[1] / 2)
        alt_y = (box_size[1] / 2) - (box_size[0] / 2)
        alt_pos = (alt_x, alt_y, 0)
        alt_rot0 = simulation_utils.eulerXYZ_to_quatXYZW((0, 0, np.pi / 2))
        alt_rot1 = simulation_utils.eulerXYZ_to_quatXYZW((0, 0, 3 * np.pi / 2))
        pose2 = simulation_utils.multiply(corner_pose, (alt_pos, alt_rot0))
        pose3 = simulation_utils.multiply(corner_pose, (alt_pos, alt_rot1))
